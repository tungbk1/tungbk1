...
def timeplay(timemusic):
   k = ''
   for i in range(len(timemusic)):
    if timemusic[i] == '_':
        while timemusic[i+1] != '.':
            k += timemusic[i+1]
            i += 1 
...
def playmusic(musicname):
    p = vlc.MediaPlayer(musicname)
    p.play()
    time.sleep(timeplay(musicname))
    p.stop()
...
def main():
	...
	try:
        ...
        playmusic("music/khongtheketnoidaudoc_11.mp3")
	...
	if start_up_successful == True:
		...
		print("Start reading !!!!!!!!!\n")
        playmusic("music/batdaudocthe_7.mp3")
	...
	while (True): 
		...
		# If NFC card is presented and NFC reader return valid data
			...
			try: 	
				...
				print("Error: "+res+" !!!!!!!!!\n")
                playmusic("music/khongtheketnoiserver_7.mp3")
			...
			if body == None and res != "Lost internet" and res != "Lost connection to OCD server":
				...
				if body != None:
                    ...
                    playmusic("music/dulieukhonghople_4.mp3")
